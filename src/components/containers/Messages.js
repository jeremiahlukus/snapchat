...
class Messages extends Component {
  ...
  loginSubmitted() {
    ...
    Turbo({ site_id: config.TURBO_APP_ID })
      .fetch("user", { username: this.state.login.username })
      .then(data => {
        if (data.length == 0) {
          message = "Register success: ";
          return Turbo({ site_id: config.TURBO_APP_ID }).createUser(
            this.state.login
          );
        } else {
      ...
      .then(data => {
        ...
        return Turbo({ site_id: config.TURBO_APP_ID }).fetch("message", {
          for: data.username
        });
      })
      .then(messages => {
  ...
  messageRead() {
    Turbo({ site_id: config.TURBO_APP_ID })
      .remove("message", this.state.activeMessage)
      .then(data => {
        let newMessages = [];
          ...
          <View style={styles.modal}>
            {this.state.modalContent === "login" ? (
              <View style={styles.login}>
                ...
                <Text>Username</Text>
                <TextInput
                  onChangeText={text => this.loginUpdated(text, "username")}
                />
                <Text>Password</Text>
                <TextInput
                  onChangeText={text => this.loginUpdated(text, "password")}
                />
                ...
                <TouchableOpacity onPress={() => this.messageRead()}>
                  ...
                  <View>
                    {this.state.activeMessage != null ? (
                      <View>
                        <Text>
                          Make sure you read your message you can only open once
                        </Text>
